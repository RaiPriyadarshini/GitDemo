...
String addCommentResponse = given().pathParam("key", "10009").log().all().header("Content-Type","application/json").body("{\r\n" +
...
//
//.multiPart("file",new File("file.txt")).when().
//
//post("rest/api/2/issue/{key}/attachments").then().log().all().assertThat().statusCode(200);

//Get Issue

String issueDetails=given().filter(session).pathParam("key", "10009")
